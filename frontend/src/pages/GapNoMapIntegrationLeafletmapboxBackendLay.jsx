...
export default function GapNoMapIntegrationLeafletmapboxBackendLay() {
  ...
  async function run() {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-map-integration-leafletmapbox-backend-lay', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', fontFamily: 'system-ui, sans-serif' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 4 }}>No map integration (Leaflet/Mapbox) backend layer</h1>
      <p style={{ color: '#666', marginBottom: 16 }}>AISatelliteImageryAnalyzer - Gap feature</p>
