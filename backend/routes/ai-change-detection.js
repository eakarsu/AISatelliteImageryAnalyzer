// AI Automated change detection
// Monitor regions, alert when significant changes detected
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AISatelliteImageryAnalyzer'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AISatelliteImageryAnalyzer. Focus area: Automated change detection. ${`Monitor regions, alert when significant changes detected`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Automated change detection.\n${`Monitor regions, alert when significant changes detected`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'change-detection', context, parsed);
    res.json({ feature: 'change-detection', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[change-detection]', err.message);
...
router.get('/history', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, endpoint, input_data, result, created_at FROM ai_results WHERE endpoint=$1 ORDER BY created_at DESC LIMIT 50', ['change-detection']);
