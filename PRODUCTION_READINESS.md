...
The supported surface is **Satellite analysis release** at `/api/governed-imagery-releases`. It provides tenant- and subject-scoped cases, opaque evidence digests, deterministic assessments, optimistic state transitions, dual control, immutable event history, and connector-failure receipts.
...
The policy endpoint reports declared connectors as unconfigured. No imagery, rights, storage, CDN, model, translation, publishing, or usage provider is configured or validated. TLS verification is mandatory when DATABASE_SSL=true.
...
2. Review `backend/migrations/001_governed_satellite_release.sql` and apply it manually in a controlled change window:
   `psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f backend/migrations/001_governed_satellite_release.sql`
